"""Общие детали демонстрационных данных.

Наполняют базу команды seed_articles, seed_contests и seed_candidate. Часть
данных у них общая: демо-кандидаты, от чьего имени записаны прохождения тестов
и решения конкурсов, и сами прохождения. Раньше это лежало копией внутри
seed_contests, а второй команде понадобилось то же самое.
"""
import random
...
def make_attempts(test, finished, max_score):
    ...
    настоящие записи — иначе статистика теста (средний балл, доля справившихся,
    брошенные попытки) считалась бы по пустой таблице.
    """
