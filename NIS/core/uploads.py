...
Раньше файлы присваивались полям модели напрямую, минуя формы, поэтому валидаторы
полей не срабатывали: на сервер уходил любой файл любого размера. Особенно опасны
.svg и .html — они отдаются с того же домена и выполняют скрипты в контексте сайта.
"""
import os
...
class UploadError(ValueError):
    """Файл не прошёл проверку. Текст сообщения можно показывать пользователю."""


def human_size(size_bytes):
    """«1.4 МБ» — для показа в интерфейсе."""
...
def validate_upload(uploaded_file, allowed_extensions, max_size):
    """Проверяет расширение и размер. Бросает UploadError с понятным текстом."""
