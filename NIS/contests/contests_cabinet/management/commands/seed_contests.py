...
# (название, уровень, категория, прохождений, вопросы)
TESTS = [
...
class Command(BaseCommand):
        ...
        with transaction.atomic():
            for title, category, level, days, prize, sub_type, hint, excerpt, case in CONTESTS:
                ...
                Contest.objects.create(
                    ...
                    participants_count=0 if finished else (days % 7) + 1,
                )
                contests += 1
            ...
            for title, level, category, submissions, questions in TESTS:
                ...
                test = Test.objects.create(
                    ...
                    stats={'level': level, 'category': category, 'submissions': submissions},
                )
    ...
    def _clear(self, username):
        ...
        self.stdout.write(self.style.SUCCESS(
            f'Удалено записей: конкурсов {contests}, тестов {tests} (со связанными страницами).'))
